...
from model.archs_bk_224_2 import bk
...
def main(data_flag, output_root, num_epochs, gpu_ids, batch_size, size, download, model_flag, resize, as_rgb, model_path, run):
  ...
  #  print(size,'1111111111')
   ...
   # print(n_channels,n_classes)
    model=bk(in_channels=n_channels,num_classes=n_classes)
...
def test(model, evaluator, data_loader, task, criterion, device, run, save_folder=None):
    ...
    with torch.no_grad():
        ...
        _,auc, acc = evaluator.evaluate(y_score, save_folder, run)
...
if __name__ == '__main__':
    ...
    l_path=['tissuemnist','pathmnist','chestmnist','dermamnist','octmnist','pneumoniamnist','retinamnist','breastmnist','bloodmnist','organamnist','organcmnist','organsmnist']
    l_size=[224]
    channels=[False,True,False,True,False,False,True,False,True,False,False,False]
